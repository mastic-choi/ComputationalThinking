...
class File_1:
    def __init__(self, file_name : str):
        ...
        try:
            ...
            self.my_file = open(self.__f_name, 'r', encoding='utf-8')
        except:
            print("\n>> 경고!! 생성자 {}이 존재하지 않음!".format(file_name))
    def getFilename(self):
        """
        private선언된 변수값에 접근하는 getter 
        """
    ...
    def setFilename(self, file_name : str):
        self.__f_name = file_name
        if D:
            print("\nsf-1) file_name set as : {}".format(self.__f_name))
    ...
    def open_mode(self, mode : str):
        ...
        try:
            if D:
                print("\no-1) open_mode() / file_name : {}, mode : {}".format(self.__f_name, mode)) 
            if mode in ['r', 'a', 'w']:
                if D:
                    print("o-2) {}을 {} mode로 open".format(self.__f_name, mode))
                self.my_file = open(self.__f_name, mode, encoding='utf-8')
    ...
    def append(self):
        ...
        try :
            if D:
                print("\na-1) append()")
                print("a-2) file name : {}".format(self.__f_name))
    ...
    def write_file(self):
        ...
        if D:
            print("\nw-1) write_file()")
            print("w-2) file name = {}".format(self.__f_name))
        try :
            if os.path.isfile(self.__f_name):
                if D:
